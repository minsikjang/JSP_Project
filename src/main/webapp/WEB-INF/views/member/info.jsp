...
<head>
	...
	<script>
		...
		function onWithdraw() {
			if (confirm("탈퇴하시겠습니까?")) {
				var url = '<%= request.getContextPath() %>/member/info.do';
		...
		function getInfo() {
			...
			if (pwd == "") {
				...
				var url = '<%= request.getContextPath() %>/member/info.do?id=${sessionScope.memberID}&password=' + pwd;
...
<body>
		...
		<main class="form-signin w-60 m-auto" align="center" style="width: 500px;">
		  ...
		  <form action="<%= request.getContextPath()%>/login/register.do" method="post" onsubmit="return submitConfirm()">
			...
			<div class="row">
			  ...
			  <div class="col">
			    <button class="btn btn-danger w-100 py-2" type="button" onclick="onWithdraw()">탈퇴</button>
			  </div>
			</div>
	...
	<!-- Modal -->
	<div class="modal fade" id="pwdModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
	  <div class="modal-dialog modal-dialog-centered">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h1 class="modal-title fs-5" id="staticBackdropLabel">비밀번호를 입력하세요.</h1>
	        <!-- <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button> -->
	      </div>
	      <div class="modal-body">
			<div class="form-floating">
		      <input type="password" class="form-control vaildation" id="pwdCheck" placeholder="패스워드" maxlength="12" onkeyup="if(window.event.keyCode==13){getInfo()}">
		      <label for="floatingPassword">비밀번호</label>
		    </div>
	      </div>
	      <div class="modal-footer">
	        ...
	        <button type="button" class="btn btn-secondary" onclick="javascript:window.history.back()">취소</button>
	      </div>
	    </div>
	  </div>
	</div>
