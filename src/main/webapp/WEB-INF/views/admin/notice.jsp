...
// ==============================================================
		...
		if((name !== null && name !== undefined && name !== '') && (subject !== null && subject !== undefined && subject !== '') && (context !== null && context !== undefined && context !== '')) {
				    ...
				    $.ajax({
				      url: "<%= request.getContextPath() %>/admin/notice.do", // 저장 처리를 담당하는 URL로 변경해야 합니다.
				      method: "POST",
				      data: data,
				      //dataType: "json",
			...
			$("#del").click(function () {
				...
				if (delIds == "") {
					alert("삭제 할 글번호를 체크하세요.");
				} else {
					if (confirm("삭제하시겠습니까?")) {
						var url = '<%= request.getContextPath() %>/admin/notice.do?delData=' + delIds.substr(1, delIds.length);
		...
		if((name !== null && name !== undefined && name !== '') && (subject !== null && subject !== undefined && subject !== '') && (context !== null && context !== undefined && context !== '')) {
				...
				if(confirm("수정하시겠습니까?")){
					// 데이터를 저장하기 위한 AJAX 요청 수행
					$.ajax({
						url: "<%= request.getContextPath() %>/admin/notice.do?idx=" + idx, // 저장 처리를 담당하는 URL로 변경해야 합니다.
						method: "POST",
						data: data,
						success: function(response) {
							// 저장 성공 시 처리
				...
				<!-- <div style="width: 1000px; margin-left: auto; margin-right: auto;">
					<table class="table" style="width: 900px; float: left;">
						<tr style="width: 100px;">
							...
							<td style="700px;"><input id="name"
								class="form-control form-control-sm" type="text" name="name"
								style="width: 100%; " maxlength="20" /></td>
						</tr>
						<tr>
							<th class="align-middle table-primary"><label for="subject">제목</label></th>
							<td colspan="2"><input id="subject" id="subject"
								class="form-control form-control-sm" type="text" name="subject"
								style="width: 100%;" maxlength="100"/></td>
						</tr>
						<tr>
							<th class="align-middle table-primary"><label for="context">내용</label></th>
							<td colspan="2"><textarea id="context"
									class="form-control form-control-sm" rows="10" name="context"
									style="resize: none; width: 100%;" maxlength="3000"></textarea></td>
						</tr>
						<tr>
							<th class="align-middle table-primary" hidden="hidden"><label for="hit"></label></th>
							<td colspan="2"><input id="hit"
								class="form-control form-control-sm" type="text" name="hit"
								style="width: 100%;"  hidden="hidden"/></td>
						</tr>
					</table>
				</div> -->
